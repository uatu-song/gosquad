#!/usr/bin/env python3
"""Build the Book 1 REVIEW COPY — the near-final manuscript for reviewers.

Derived from build_arc_epub.py. Same text pipeline; different front matter:
a review-copy notice instead of an uncorrected-proof notice, a version line,
and a contents page. Text is byte-identical to the gated drafts. This script
formats; it never edits.

ORIGINAL DOCSTRING OF THE ARC BUILDER FOLLOWS.
...
OUT = HERE / 'GoSquad_Book1_REVIEW.epub'
...
def blocks(path):
    ...
    for line in t.split('\n'):
        s = line.strip()
        if not s or HEADER.match(s) or EPI.match(s):
            continue
...
print(f'\nREVIEW COPY v{VERSION}: {len(CHAPTERS)} chapters + epilogue, ~{total:,} words')
print(f'{OUT}  ({OUT.stat().st_size//1024} KB)  v{VERSION}  {STAMP}  (git {REV})')
